...
fn run(args: Args) -> Result<()> {
		...
		for (timestep_index, &timestep) in timesteps.iter().enumerate() {
			...
			//let skip = 32;
		 	//let mask = map.gt((timestep_index+skip) as f32/(n_steps+skip) as f32)?.squeeze(0)?.to_dtype(dtype)?;
			...
			//latents = ((&initial_latents/*_with_noise_t*/ * mask.clone())? + ((1.-mask) * latents)?)?;
